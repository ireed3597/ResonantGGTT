...
def fitSignalModel(df, mx, outdir, my=125):  
  ...
  #popt, perr = fitDCB(df[(df.MX==mx)&(df.MY==my)], fit_range=[my-10,my+10], savepath=os.path.join(outdir, "mx_%d_my_%d.png"%(mx,my)))
...
def deriveModels(original_df, proc_dict, optim_results, original_outdir):
  ...
  all_masses = np.array([common.get_MX_MY(entry["sig_proc"]) for entry in optim_results])
  ...
  #for year in [2016]:
    ...
    for entry in optim_results[:]:
      ...
      to_fit_masses = [[mx, my] for mx in to_fit_mxs for my in to_fit_mys] #convenient to leave as list for tagSignals
      ...
      #for SR in [0]:
        ...
        #   popt_check, perr_check = fitSignalModel(df, closest, outdir)
