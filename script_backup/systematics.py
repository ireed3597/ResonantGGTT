...
def deriveInterpolationSystematic(df, nominal_masses, masses):
  ...
  splines = [spi.interp1d(nom_m[:i]+nom_m[i+1:], norms_nominal[:i]+norms_nominal[i+1:], kind='linear') for i in range(1, len(nominal_masses)-1)]
  
  norms_nominal = np.array(norms_nominal) 
  norms_interpolated = np.array([splines[i](m) for i, m in enumerate(nominal_masses[1:-1])])

  variations = 1 + abs(1 - norms_nominal[1:-1]/norms_interpolated)
...
def loadDataFrame(path, proc_dict, optim_results, columns=None, batch_size=None):
...
def loadDataFrames(args):
  ...
  df = loadDataFrame(os.path.join(args.parquet_input, "merged_nominal.parquet"), proc_dict, optim_results, batch_size=None)
  ...
  for path in os.listdir(args.parquet_input):
    if (".parquet" in path) and ("nominal" not in path):
      print(path)
      df = loadDataFrame(os.path.join(args.parquet_input, path), proc_dict, optim_results, columns=systematic_columns, batch_size=None)
