...
def plotSigEff(mx, sig_eff, interp_uncert, savename, SR):
  s = interp_uncert!=1
  ...
  plt.fill_between(mx[s], l, h, label="Interpolation Uncert.", alpha=0.2)
  plt.xlabel(r"$m_X$")
...
for i, SR in enumerate(cats):
  ...
  plotSigEff(mx, sig_eff_year_avg[i,:], inter_uncert_year_avg[i,:], "sig_eff_cat%d.pdf"%SR, SR)

  for j, year in enumerate(years):
   plotInterpUncert(mx, inter_uncert[j,i,:], "interpolation_uncertainty_%d_cat%d"%(year, SR))
   plotSigEff(mx, sig_eff[j,i,:], inter_uncert[j,i,:], "sig_eff_%d_cat%d.pdf"%(year, SR), SR)
